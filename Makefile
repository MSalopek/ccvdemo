...
HTTPS_GIT := github.com/msalopek/ccvdemo.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=ccvdemo \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=consumerd \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/consumerd ./cmd/consumerd
endif

build-windows-client: go.sum
	GOOS=windows GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o build/consumerd.exe ./cmd/consumerd
...
draw-deps:
	...
	@goviz -i ./cmd/consumerd -d 2 | dot -Tpng -o dependency-graph.png
...
ifeq (,$(shell which heighliner))
	...
	heighliner build -c ccvdemo --local -f chains.yaml
endif
...
set-testnet-configs:
	consumerd config set client chain-id localchain-1
	consumerd config set client keyring-backend test
	consumerd config set client output text

# import keys from testnet.json into test keyring
setup-testnet-keys:
	-`echo "decorate bright ozone fork gallery riot bus exhaust worth way bone indoor calm squirrel merry zero scheme cotton until shop any excess stage laundry" | consumerd keys add acc0 --recover`
	-`echo "wealth flavor believe regret funny network recall kiss grape useless pepper cram hint member few certain unveil rather brick bargain curious require crowd raise" | consumerd keys add acc1 --recover`
