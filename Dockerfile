...
# then log output of file /code/bin/consumerd
# then ensure static linking
RUN LEDGER_ENABLED=false BUILD_TAGS=muslc LINK_STATICALLY=true make build \
  && file /code/build/consumerd \
  && echo "Ensuring binary is statically linked ..." \
  && (file /code/build/consumerd | grep "statically linked")

# --------------------------------------------------------
FROM alpine:3.16

COPY --from=go-builder /code/build/consumerd /usr/bin/consumerd
